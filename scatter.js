function drawScatter(allMonthsData, monthToShow, dataType, sizes) {
  ...
  const yScale = setScatterScale(
    allMonthsData,
    sizes.scatter,
    "y",
    "medianAge"
  );
  const rScale = setScatterScale(
    allMonthsData,
    sizes.scatter,
    "r",
    "vaccines",
...
function cleanScatterData(monthData, dataType) {
  ...
  return monthData.filter(
    (ctry) => !isNaN(ctry[dataType]) && ctry.medianAge && ctry.devIndex
  );
}
...
function setScatterScale(allMonthsData, scatterSizes, dimension, key, min) {
  ...
  return d3.scaleLinear().domain([min, max]).range(scope);
}

function drawScatterAxes(xScale, yScale, scatterSizes, dataType) {
  const { height, padding } = scatterSizes;

  d3.select(".scatter__chart").selectAll(".axis").remove();
...
function scatterLegendVac(allMonthsData, scatterSizes, paddings) {
  const vacRadii = setScatterScale(
    allMonthsData,
    scatterSizes,
    "r",
    "vaccines",
  ...
  const vacRange = setScatterScale(
    allMonthsData,
    scatterSizes,
    "r",
    "vaccines",
...
function scatterLegendDev(allMonthsData, scatterSizes, paddings) {
  const devColors = setScatterScale(
    allMonthsData,
    scatterSizes,
    "f",
    "devIndex"
  ).range();
  const devRange = setScatterScale(
    allMonthsData,
    scatterSizes,
    "f",
    "devIndex"
  ...
  d3.select(".scatter__legend").append("defs").html(`
